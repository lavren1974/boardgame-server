...
const delaybetweenGamesSec = 50;
const turnTimeFastSec = 100;
...
interface State {
    ...
    // Текущие подключенные пользователи или зарезервированные места.
    presences: { [userId: string]: nkruntime.Presence | null }
    // Number of users currently in the process of connecting to the match.
    // Количество пользователей, которые в данный момент находятся в процессе подключения к матчу.
    joinsInProgress: number
    // True if there's a game currently in progress.
    // Истинно, если в данный момент идет игра.
    playing: boolean
...
let matchLoop: nkruntime.MatchLoopFunction<State> = function (ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, dispatcher: nkruntime.MatchDispatcher, tick: number, state: State, messages: nkruntime.MatchMessage[]) {
    ...
    if (!state.playing) {
        ...
        state.playing = true;
        state.board = new Array(25); //TTTStars
        state.marks = {};
